...
- `STRING` 在语义层按 UTF-8 byte string literal 处理；未转义源码文本按 UTF-8 编码写入，`"..."` 是 `u8 const[*]`，并自动带一个结尾 `0` 字节。
- `CHAR` / `STRING` 当前支持这些转义：`\n`、`\t`、`\r`、`\0`、`\\`、`\'`、`\"`、`\xN`、`\xNN`。
- `\xN` / `\xNN` 表示一个原始字节；单个十六进制数字时会自动补成 `\x0N`。它既可用于普通字节串，也可显式写出 UTF-8 字节序列；字符串整体不做 UTF-8 合法性校验。
...
此外，直接函数指针值变量要求在 `var` 定义时立刻给出初始化值；固定数组如果元素类型是直接函数指针值，也要求完整初始化，避免缺省补零形成 null 函数指针。像 `(i32: i32)*` 这类外层再包一层普通指针的类型，当前仍按普通指针存储处理。
