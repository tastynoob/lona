...
5. [trait.md](trait.md): `trait`、`impl Type: Trait`、`impl Trait for Type { ... }`、`value.Trait.method(...)`、`Trait dyn` 和动态 / 静态分派边界。
...
- [trait.md](trait.md): trait 声明、impl header / impl body、显式 trait 路径调用和 `Trait dyn`。
