...
makeInjectedMemberArrayDimension(std::int64_t value) {
    AstToken token(TokenType::ConstInt32, std::to_string(value).c_str(),
                   location());
...
isInjectedMemberBitsArrayType(TypeClass *type,
    ...
    if (!array || array->getElementType() != u8Ty ||
...
canExplicitBitCopy(TypeClass *targetType, TypeClass *sourceType) {
    ...
    const bool sourceBitsArray = isInjectedMemberBitsArrayType(sourceType);
    const bool targetBitsArray = isInjectedMemberBitsArrayType(targetType);
...
commonNumericType(TypeTable *typeTable, TypeClass *leftType,
    ...
    if (isInjectedMemberFloatFamily(leftType) &&
        ...
        return injectedMemberFloatTypeForWidth(
            std::max(injectedMemberNumericBitWidth(typeTable, leftType),
    ...
    if (isInjectedMemberIntegerFamily(leftType) &&
        ...
        auto *unsignedType =
            isUnsignedIntegerType(leftType) ? leftType : rightType;
...
resolveInjectedMember(TypeTable *typeTable, TypeClass *receiverType,
    ...
    if (memberName == "tobits") {
        ...
        auto byteCount = static_cast<std::int64_t>(
            typeTable->getTypeAllocSize(receiverType));
