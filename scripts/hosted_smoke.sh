...
bash "$BUILD_HOSTED" "$main_program" "$main_exe"
bash "$BUILD_HOSTED" "$top_level_program" "$top_level_exe"
...
if [ "$main_status" -ne 9 ]; then
    echo "expected hosted main program to exit with 9, got $main_status" >&2
    exit 1
fi

if [ "$top_level_status" -ne 0 ]; then
    echo "expected hosted top-level program to exit with 0, got $top_level_status" >&2
    exit 1
fi
